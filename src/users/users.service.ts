...
export class UsersService {
  ...
  /** Admin 後台：使用者列表（可依角色篩選；service_role 繞過 RLS 讀取全部 profiles） */
  async list(page: number, pageSize: number, role?: string) {
    ...
    let builder = this.supabase.admin
      .from('profiles')
      .select(
        'id, email, full_name, display_name, role, agency_name, created_at',
        { count: 'exact' },
      )
  ...
  /** Admin 刪除用戶：不能刪自己；名下還有物件的 agent 要先處理物件 */
  ...
  /** Admin 設定使用者角色（buyer / agent）；不能改自己以免鎖死後台 */
  async updateRole(actorId: string, userId: string, role: 'buyer' | 'agent') {
    if (actorId === userId) {
